...
def combined_penalty(predictions, alpha, beta, lambda_tv, lambda_const):
    ...
    return (lambda_tv * tv_loss + lambda_const * const_loss) / (
        lambda_tv + lambda_const
    )


def blocky_loss(
    alpha=8.0, beta=1.0, lambda_tv=1.0, lambda_const=1.0, lambda_val=2.6157
):
    ...
    print(
        f"blocky loss params alpha={alpha} beta={beta} lambda_tv={lambda_tv} lambda_const={lambda_const} lambda_val={lambda_val}"
    )
    ...
    def loss_fn(predictions, targets, epoch=1):
        ...
        smoothness_loss = combined_penalty(
            predictions, alpha, beta, lambda_tv, lambda_const
        )
