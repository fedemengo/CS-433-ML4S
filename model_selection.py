...
optuna.logging.get_logger("optuna").addHandler(
    logging.FileHandler(f"./logs/optuna-{datetime.now().strftime('%Y%m%d_%H%M%S')}.log")
)
...
def prepare_bold_input(X_data):
    ...
    X_scaled = np.vstack(
        [RobustScaler().fit_transform(ts.reshape(-1, 1)).ravel() for ts in X_flat]
    )
...
def select_model(models_and_trainers, X_train, y_train, n_trials=100, n_folds=5):
    ...
    for ModelClass, TrainerClass in models_and_trainers:
        ...
        try:
            ...
            study.optimize(
                objective, n_trials=n_trials, callbacks=[log_trial], catch=(Exception,)
            )
        study.trials_dataframe().to_csv(
            f'./optuna/optuna_trials_{datetime.now().strftime("%Y%m%d_%H%M%S")}.csv'
        )
