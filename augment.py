...
def shift(X, y, shift_range=(-20, +20), groups=5, ratio=0.3):
    ...
    for g in range(groups):
        ...
        shift_indices = np.random.choice(
            range(start_idx, end_idx), size=num_to_shift, replace=False
        )
...
def temporal_scale(X, y, scale_range=(0.8, 1.2), ratio=0.3, keep_len=True):
    ...
    if not keep_len:
        ...
        scaled_X = torch.nn.functional.interpolate(
            X.transpose(1, 2), size=new_length
        ).transpose(1, 2)
        scaled_y = torch.nn.functional.interpolate(
            y.unsqueeze(1), size=new_length
        ).squeeze(1)
    else:
        scaled_X, scaled_y = X.clone(), y.clone()

    for vox_idx in np.random.choice(
        range(n_voxels), size=int(n_voxels * ratio), replace=False
    ):
        new_length = int(time_points * np.random.uniform(*scale_range))
        x_interp = (
        ...
        y_interp = torch.nn.functional.interpolate(
            y[vox_idx].unsqueeze(0).unsqueeze(0), size=new_length
        ).squeeze()
...
def augment_data(
    ...
    for i in range(num_samples):
        ...
        noise_X = torch.normal(
            mean=0, std=noise_std_value, size=scaled_X.shape, device=scaled_X.device
        )
        noisy_X = scaled_X + noise_X

        if plot_times:
            noise_info = 20 * np.log10(
                (noise_level**2 / torch.mean(scaled_X**2).item())
            )
