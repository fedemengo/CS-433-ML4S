from utils import project_dir, paradigm_dir, derivatives_dir, anat_dir, DATASET_DIR
...
from utils import subject_gm_mask_path, subject_task_active_mask_path, subject_task_regressor_path, subject_task_sample_prefix
...
from preprocessing import SMOOTHING, TR
from preprocessing import (
    subject_run,
    binary_design_matrix, 
    get_predicted_neural_activity,
...
motor_atlas_old = f"{project_dir}/atlas/HMAT/HMAT.nii"
...
def get_block_and_neural_activity(glm, active_mask_data, binary_design_matrix,original_data):
    ...
    return original_data_masked, predicted_neural_activity, predicted_dat
...
def main():
    ...
    for entry in subjects_kept:
        ...
        X_list, Y_list, Y_conv_list = prepare_subject_sample(
            subject_id, task, acquisition, SMOOTHING, voxel_quantile=98,
    ...
    final_dataset.to_netcdf(pjoin(DATASET_DIR,f'dataset_{task}_{len(subjects_kept)}_subjects_both_noncens.nc'))
